...
//http://community.topcoder.com/stat?c=problem_statement&pm=1773&rd=4570
...
public class PickTeam 
{
	...
	List<String>  searched  = new LinkedList<String>();
	
	int max = Integer.MIN_VALUE;
	public String[] pickPeople(int teamSize, String[] people)
	{
		int[][] matrix = new int[people.length][people.length];
		String[] pplArray = new String[people.length];
		for(int i = 0 ; i < people.length ; i++)
		{
			String ppl = people[i];
			String[] arr = ppl.split("\\s+");
			pplArray[i] =  arr[0];
			for(int j = 1 ; j < arr.length; j++)
			{
				matrix[i][j-1] = Integer.parseInt(arr[j]);
			}
		}
        this.matrix = matrix;		
		for(int i = 0 ; i < people.length ; i++)
        ...
        {
        	result[i] = pplArray[finalList.get(i)];
        }
		return result;
	...
	private void dfs(int source,
			         ...
			         int currentSum) 
	{
		String str = getString(tempList);
		if (currentCount == teamSize-1
			&& !searched.contains(str))
		{
			if (currentSum > max)
			{
				max = currentSum;
				finalList = new LinkedList<Integer>(tempList);
			}
			String str1 = getString(tempList);
			searched.add(str1);
			return;
		}
		else if (searched.contains(str))
		{
			return;
		}
...
//				 "E -5 3 -8 8 -2 -2 3 4 4 4 4 -2 -6 1 3 -7 5 8 1 0"};
	    ...
	    String[] result = new PickTeam().pickPeople(9, arr);
